...
# Put both the repo root (for `import src.*`) and src/ (so build_dataset.py's
# `from light_curve import ...` resolves) on the path before importing.
REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.append(str(REPO_ROOT))
sys.path.append(str(REPO_ROOT / "src"))
from src.build_gb_dataset import (
    get_star_labels,
    pick_all_stars,
    download_lightcurve,
