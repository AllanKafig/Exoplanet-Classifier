...
class ExoplanetRNN():
    """
    Classifies light curves based on if they belong to star with a transiting 
    exoplanet (input 1, hidden 32, 1 logit).
    """
...
def bce_loss(logits, y, pos_weight=1.0, eps=1e-7):
    """BCE with a positive-class weight, by hand (no nn loss)."""
...
def sgd_step(params, lr):
    """Manual gradient-descent update over all parameters: w = w - lr * grad."""
    with torch.no_grad():
        for w in params:
            w -= lr * w.grad
            w.grad.zero_()
...
def train_rnn(light_curves, labels, epochs, batch_size, learning_rate):
    """Train the GRU with gradient descent."""
    ...
    for epoch in range(epochs):
        ...
        for batch_x, batch_y in loader:
            ...
            sgd_step(model.parameters(), learning_rate)     # manual gradient descent
...
if __name__ == "__main__":
    ...
    #standardize the light curves to have mean 0 and std 1 for better training stability
    light_curves = standardize_rows(light_curves)  
    ...
    model = train_rnn(
        ...
        labels=y_train,
        epochs=80,   # test metrics keep rising to ~80-90 epochs, then plateau
        batch_size=64,
        learning_rate=LEARNING_RATE,
    )

    prob = evaluate(model, x_test)
    print("\nGradient-descent (SGD) model performance:")
    ...
    torch.save([w.detach().cpu() for w in model.parameters()], "exoplanet_rnn_v2.pt")
